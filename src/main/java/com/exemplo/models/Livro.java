...
public class Livro {
    ...
    public static void editarLivro(Livro livro, String isbn, String titulo, String autor,
        ...
        if (numeroCopias < 1) {
            throw new RuntimeException("Erro: O número total de cópias deve ser no mínimo 1.");
        }

        if (disponiveis < 0) {
            throw new RuntimeException("Erro: Disponíveis não pode ser negativo.");
        }

        if (disponiveis > numeroCopias) {
            throw new RuntimeException("Erro: Disponíveis não pode ser maior que o número de cópias.");
        }

        if (anoPublicacao < 1400 || anoPublicacao > 2100) {
            throw new RuntimeException("Erro: Ano de publicação inválido!");
        }
        livro.setIsbn(isbn);
