...
public class Validation {
   ...
   public static boolean isValid(String telefone, String email, String endereco, String nome, String cpf, String senha) {
        if (!isNameValid(nome)) return false;
        if (!isCpfValid(cpf)) return false;
        if (!isEmailValid(email)) return false;
        if (!isSenhaValid(senha)) return false; 
        if (!isTelefoneValid(telefone)) return false;
        if (!isEnderecoValid(endereco)) return false;     
